...
#pragma warning(default : 6031)
	...
	PARSER psr = CreateParser(L"LanClientConfig.txt");
	WCHAR ipStr[16];
		...
		do
		{
			...
			if (!bGQCSRet && pOverlapped)
			{
				...
				if (pOverlapped->why == OVERLAPPED_REASON::CONNECT)
				{
					...
					pLanClient->DisconnectStack_.Push((short)(pSession - pLanClient->pSessionArr_));
					if (errCode != 0)
			...
			case OVERLAPPED_REASON::DISCONNECT:
				bContinue = true;
				pLanClient->DisconnectProc(pSession);
				break;
			default:
				__debugbreak();
			}
